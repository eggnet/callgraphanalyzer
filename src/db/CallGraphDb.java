...
public class CallGraphDb extends DbConnection
{
	...
	public List<Change> getAllFileOwnerChangesBefore(String FileId, String CommitId)
	{
		...
		{
			LinkedList<Change> changes = new LinkedList<Change>();
			String sql = "SELECT commit_id, file_id, owner_id, line_start, line_end, change_type FROM owners natural join commits where commit_date <= (select commit_date from commits where commit_id=?)" +
			...
			while(rs.next())
			{
				changes.add(new Change(rs.getString("owner_id"), rs.getString("commit_id"), Resources.ChangeType.valueOf(rs.getString("change_type")), rs.getString("file_id"), rs.getInt("line_start"), 
						rs.getInt("line_end")));
			}
			return changes;
		}
